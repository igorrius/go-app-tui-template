...
Provides embedded SQL schema migration management via `pressly/goose/v3`. Migration files are compiled into the binary using `//go:embed` and applied explicitly through the `migrate` CLI subcommand.
