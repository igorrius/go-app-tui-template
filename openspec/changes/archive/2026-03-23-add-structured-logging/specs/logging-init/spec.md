...
The system SHALL provide a `logging.Init(cfg LoggingConfig, bus *vein.Bus) error` function that creates a `VeinHandler`, starts the `AsyncWriter` goroutine, and calls `slog.SetDefault` with the resulting logger.
