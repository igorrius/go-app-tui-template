...
The system SHALL provide a `LogView` screen that subscribes to `LogEvent` messages from the vein Dispatcher and renders each received log entry as a formatted text line using `logging.TextFormatter`.

#### Scenario: LogView renders received log events
- **WHEN** a `LogEvent` is published to the vein Dispatcher
